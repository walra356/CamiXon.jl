...
[`fdiff_expansion_weights(coeff, ∇(k))`](@ref)
`` → \bar{B}^{k} ≡ [B_k^k,⋯\ B_0^k]``
...
```
where ``s^k_x[j] ≡ s^k_{k-j}(x)``. The weights vector ``s^k_x`` is generated by the function [`bwd_diff_expansion_weights(coeffs,∇)`](@ref), with `∇ =` [`fdiff_weights_array(k)`](@ref).

Functions:

`β` = [`fdiff_expansion_coeffs_differentiation(k,x)`](@ref) ``→ [l_0^{\prime}(x),⋯\ l_k^{\prime}(x)]``
...
`β` = [`fdiff_expansion_coeffs_adams_bashford(k)`](@ref) ``→ [B_k^k(x),⋯\ B_0^k(x)]``
