...
=[c_{k}^{k},\thinspace c_{k-1}^{k},\thinspace\ldots,c_{0}^{k}]\left[\begin{array}{c}
f[n]\\
\vdots\\
f[n+k]
\end{array}\right].
...
In this notation the  ``k^{th}``-*order backward difference* (which
involves ``k+1`` points) is defined by a *weighted sum* over the function
values in backward order, ``f[n],\ \ldots,\ f[n-k]``,
...
```math
\sum_{p=0}^{\infty}α_{p}Δ^{p}f[n]=\sum_{p=0}^{k}α_{p}\nabla^{p}f[n]+\cdots.
```
...
``b_{j}^{k}=\sum_{p=j}^{k}α_{p}c_{j}^{p}``, with ``j=0,\ \ldots,\ k``.
By a change of dummy index we write the summation in *forward order*

```math
\sum_{p=0}^{k}α_{p}Δ^{p}f[n]=\sum_{j=0}^{k}b_{k-j}^kf[n-k+j].
```

Note that the *weights vector* ``b^k`` contains the weights in backward order.
It is generated by the function [`f_diff_expansion_weights(coeffs,∇)`](@ref),
where `∇ =`[` f_diff_weights_array(kmax)`](@ref).

Functions:

`coeffs` = ``[α_0,\ \ldots,\ α_k]``

[`f_diff_expansion_weights(coeffs,∇)`](@ref)
                          ``\rightarrow b^k \equiv [b_k^k,\ ,\ldots,\ b_0^k]``
...
```math
\sum_{p=0}^{\infty}α_{p}∇^{p}f[n]=\sum_{p=0}^{k}α_{p}∇^{p}f[n]+\cdots.
```
...
where the weighted summation is defined by the *weights* ``b_{j}^{k}=\sum_{p=j}^{k}α_{p}c_{j}^{p}``, with ``j=0,\ \ldots,\ k``. By a change of dummy index we write the summation in *forward order*
