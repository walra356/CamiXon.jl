...
=\sum_{p=0}^{k}α_{p}\sum_{j=0}^{p}c_{j}^{i}f[n-j]
...
```math
\sum_{p=0}^{k}α_{p}Δ^{p}f[n]=\sum_{j=0}^{k}F_{k-j}^kf[n-k+j].
```

Note that the *weights vector* ``F^k = F_0^k ≡ [F_k^k,\ ,⋯,\ F_k^k]``
contains the weights in forward order. It is generated by the function
