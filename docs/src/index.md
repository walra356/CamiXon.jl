...
```math
\nabla^k f[n] = \sum_{j=0}^{k} c^k_[j]f[n-k+j],
```
...
```
where ``r^k_x[j] ≡ r_{k-j}(x)^k``. The weights vector ``r^k_x`` is generated by the function [`f_diff_expansion_weights(coeffs,∇)`](@ref), with `∇ =` [`f_diff_weights_array(k)`](@ref).
...
```math
\frac{df}{dx}[n+x]=\sum_{p=0}^{k}l_{p}^{\prime}(x)\nabla^{p}f[n]= \sum_{j=0}^{k}s_{k-j}^k(x)f[n-k+j],
```
where ``s^k_x[j] ≡ s_{k-j}(x)^k``. The weights vector ``s^k_x`` is generated by the function [`f_diff_expansion_weights(coeffs,∇)`](@ref), with `∇ =` [`f_diff_weights_array(k)`](@ref).
