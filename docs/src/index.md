...
`` → \bar{c} ≡ [\bar{c}^0,\ \bar{c}^1,⋯\ \bar{c}^{kmax} ]``
...
[`f_diff_weights_array(kmax)`](@ref)
`` → \bar{c}^k ≡ [\bar{c}^0,\ \bar{c}^1,⋯\ \bar{c}^{kmax} ]``
...
where `\bar{c} =`[` f_diff_weights_array(kmax)`](@ref).

Functions:

[`fwd_diff_expansion_weights(α,\bar{c})`](@ref) ``→ F^k ≡ [F_0^k,⋯\ F_k^k]``
...
the function [`bwd_diff_expansion_weights(β,\bar{c})`](@ref),
where `\bar{c} ≡`[` f_diff_weights_array(kmax)`](@ref).

Functions:

[`bwd_diff_expansion_weights(β,\bar{c})`](@ref)
`` → \bar{B}^{k} ≡ [B_k^k,⋯\ B_0^k]``
