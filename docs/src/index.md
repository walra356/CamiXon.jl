...
```
where ``s^k_x[j] ≡ s^k_{k-j}(x)``. The weights vector ``s^k_x`` is generated by
the function [`fdiff_expansion_weights(coeffs, fdiffs; notation=bwd)`](@ref).
...
[`fdiff_expansion_weights(β, fdiffs; notation=bwd)`](@ref)
