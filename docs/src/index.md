...
Note that the *weights vector* ``F^k`` contains the weights in backward order.
It is generated by the function [`forward_diff_expansion_weights(α,Δ)`](@ref),
where `Δ =`[` f_diff_weights_array(kmax)`](@ref).

Functions:

[`forward_diff_expansion_weights(α,Δ)`](@ref)
                          ``\rightarrow F^k \equiv [F_k^k,\ ,\ldots,\ F_0^k]``
...
A finite-difference expansion truncated order ``k`` is defined
by ``k+1`` *finite-difference expansion coefficients*, above denoted
by ``β_{0},\ \ldots,\ β_{k}``. Note that both the coefficients and the
operators appear in forward order. It takes some bookkeeping to rewrite the
expansion in the form of a *weighted sum* over the ``k+1`` *function values*
``f[n],\ldots,f[n-k]`` (note the *backward order*). Substituting the finite
...
where the weighted summation is defined by the *weights* ``B_{j}^{k}=\sum_{p=j}^{k}β_{p}c_{j}^{p}``, with ``j=0,\ \ldots,\ k``. By a change of dummy index we write the summation in *forward order*

```math
\sum_{p=0}^{k}β_{p}\nabla^{p}f[n]=\sum_{j=0}^{k}B_{k-j}^kf[n-k+j].
```

Note that the *weights vector* ``B^k`` contains the weights in backward order.
It is generated by the function [`backward_diff_expansion_weights(β,∇)`](@ref),
...
[`backward_diff_expansion_weights(β,∇)`](@ref) ``\rightarrow b^k \equiv [B_k^k,\ ,\ldots,\ B_0^k]``
...
```
where ``s^k_x[j] ≡ s^k_{k-j}(x)``. The weights vector ``s^k_x`` is generated by the function [`backward_diff_expansion_weights(coeffs,∇)`](@ref), with `∇ =` [`f_diff_weights_array(k)`](@ref).
