...
For given forward-difference *expansion coefficients*  
``α`` the corresponding  *expansion weights* ``F^k`` are generated by
the function [`fwd_diff_expansion_weights(α,Δ)`](@ref),
where `Δ =`[` f_diff_weights_array(kmax)`](@ref).
...
For given backward-difference *expansion coefficients*  ``β = [β_0,⋯\ β_k]``
the correspoding *expansion weights* ``\bar{B}^k`` are generated by
the function [`bwd_diff_expansion_weights(β,∇)`](@ref),
where `∇ =`[` f_diff_weights_array(kmax)`](@ref).

Functions:

[`bwd_diff_expansion_weights(β,∇)`](@ref)``→ \bar{B}^{k} ≡ [B_k^k,⋯\ B_0^k]``
...
\end{cases},
```
where ``(x)_{p}`` is the [`pochhammer`](@ref) symbol. Evaluating the
finite-difference expansion up to order ``k`` we obtain

```math
f[n+x] =\sum_{p=0}^{k}(x)_{p}∇^pf[n]= \sum_{j=0}^{k}r_j^k(x)f[n-j],
```

where the ``k+1`` *weights* ``r_j^k(x)= \sum_{p=j}^{k} (x)_p c_j^p`` are the  *lagrangian interpolation weights*.  After changing dummy index to obtain forward order the expansion becomes
```math
f[n+x] =\sum_{p=0}^{k}(x)_p∇^pf[n]= \sum_{j=0}^{k}r_x^k[j]f[n-k+j],
```
where ``r^k_x[j] ≡ r_{k-j}^k(x)``. The weights vector ``r^k_x`` is generated by the function [`bwd_diff_expansion_weights(β,∇)`](@ref), with `∇ =` [`f_diff_weights_array(k)`](@ref).
...
```
where ``s^k_x[j] ≡ s^k_{k-j}(x)``. The weights vector ``s^k_x`` is generated by the function [`bwd_diff_expansion_weights(coeffs,∇)`](@ref), with `∇ =` [`f_diff_weights_array(k)`](@ref).

Functions:

`β` = [`f_diff_expansion_coeffs_differentiation(k,x)`](@ref) ``→ [l_0^{\prime}(x),⋯\ l_k^{\prime}(x)]``

`Lagrange differentiation weights` = [`bwd_diff_expansion_weights(β,∇)`](@ref) ``s^k_x[j] ≡ → [s_k^k(x),⋯\ s_0^k(x)]``
...
`adams_bashford_integration_weights` = [`bwd_diff_expansion_weights(β,∇)`](@ref) ``→ [A_k^k(x),⋯\ A_0^k(x)]``
...
where the ``a_j^k(x)= \sum_{p=j}^{k} b_pc_j^p`` are the ``(k+1)``-point *Adams-Moulton integration weights*. These are generated in *backward order* by the function [`bwd_diff_expansion_weights(coeffs,∇)`](@ref), with `∇ = `[`f_diff_weights_array(k)`](@ref).

Functions:

`β` = [`f_diff_expansion_coeffs_adams_moulton(k)`](@ref) ``→ [b_0,⋯\ b_k]``

`adams_moulton_weights` = [`bwd_diff_expansion_weights(β,∇)`](@ref)``→ [a_k^k,⋯\ a_0^k]``
