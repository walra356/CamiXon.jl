...
```
are the *lagrangian interpolation weights* corresponding to the point f[n-x].  
The weights vector ``F^k(x)`` is generated by
the function [`fwd_diff_expansion_weights(α,Δ)`](@ref),
with `Δ =` [`f_diff_weights_array(k)`](@ref).

Functions:

`α` = [`f_diff_expansion_coeffs_lagrange(k,x)`](@ref)
``→ α ≡ [α_0(x),⋯\ α_k(x)]``
