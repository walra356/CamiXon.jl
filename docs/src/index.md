...
```
where ``s^k_x[j] ≡ s^k_{k-j}(x)``. The weights vector ``s^k_x`` is generated by the function [`fdiff_expansion_weights(coeffs,∇(k))`](@ref).

Functions:

`β` = [`fdiff_expansion_coeffs_differentiation(k,x)`](@ref) ``→ [l_0^{\prime}(x),⋯\ l_k^{\prime}(x)]``

`Lagrange differentiation weights` = [`fdiff_expansion_weights(coeffs,∇(k))`](@ref) ``s^k_x[j] ≡ → [s_k^k(x),⋯\ s_0^k(x)]``
...
`adams_bashford_integration_weights` = [`fdiff_expansion_weights(β,∇(k))`](@ref) ``→ [A_k^k(x),⋯\ A_0^k(x)]``
...
`adams_moulton_weights` = [`fdiff_expansion_weights(β,∇(k))`](@ref)``→ [a_k^k,⋯\ a_0^k]``
