...
by ``α_{0},⋯\ α_{k}``. It takes some bookkeeping to rewrite the
...
```math
\sum_{p=0}^{k}α_{p}Δ^{p}f[n]
=\sum_{p=0}^{k}α_{p}\sum_{j=0}^{p}c_{j}^{p}f[n-j]
=\sum_{j=0}^{k}\sum_{p=j}^{k}α_{p}c_{j}^{p}f[n-j]
...
where ``F^k  ≡ [F_0^k,⋯\ F_k^k]``. By a change of dummy index we obtain

```math
\sum_{p=0}^{k}α_{p}Δ^{p}f[n]
=\sum_{j=0}^{k}F_{k-j}^{k}f[n-k+j]=\bar{F}^k \cdot f[n-k:n],
```

with ``\bar{F}^k ≡ [F_k^k,⋯\ F_0^k]``.
The *forward difference expansion weights* are generated by the function
[`fwd_diff_expansion_weights(α,Δ)`](@ref),
where ``α = [α_0,⋯\ α_k]`` and `Δ =`[` f_diff_weights_array(kmax)`](@ref).
...
the weights in backward order. It is generated by the function
[`bwd_diff_expansion_weights(β,∇)`](@ref),
where ``β = [β_0,⋯\ β_k]`` and `∇ =`[` f_diff_weights_array(kmax)`](@ref).

In general (for ``β ≠ α``) there is *no simple symmetry relation* between
