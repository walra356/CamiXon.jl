...
Important: note that `Δ = ∇` (although we will continue to use both symbols,
`Δ` when using the forward notation and `∇` when using the backward notation)
...
``f[n-x]``. The weights vector ``F^k(x)`` is generated by
the function [`fwd_diff_expansion_weights(α,Δ)`](@ref),
with `Δ ≡` [`f_diff_weights_array(k)`](@ref).
...
[`fwd_diff_expansion_weights(α,Δ)`](@ref)
``→ F^k(x) ≡ [F^k_0(x),⋯\ F^k_k]``
...
```math
f[n+x] =\sum_{p=0}^{k}β_p(x)∇^pf[n]= \sum_{j=0}^{k}r_x^k[j]f[n-k+j],
```
where ``r^k_x[j] ≡ r_{k-j}^k(x)``. The weights vector ``r^k_x`` is generated by
the function [`bwd_diff_expansion_weights(β,∇)`](@ref),
with `∇ ≡ ` [`f_diff_weights_array(k)`](@ref).
