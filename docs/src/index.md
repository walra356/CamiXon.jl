...
The corresponding coefficient vector is given by the
...
```math
 ...
 ``` 
are the ``k^{th}``-order lagrangian differentiation weights, with the weights
vector ``B^k(x)`` generated by the function
[`fdiff_expansion_weights(coeffs, bwd, reg)`](@ref)..  After changing
...
=\bar{B}^k(x) ⋅ f[n-k:n].,
```
where the weights vector ``\bar{B}^k(x)`` is generated by
the function [`fdiff_expansion_weights(coeffs, bwd, rev)`](@ref).
