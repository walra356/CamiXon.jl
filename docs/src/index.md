...
Consider analytic function \$f\f tabulated in *normal ordering*, denoted by \$[f[1],\ldots,f[n]]\$, where the indices point to the positions $x[1],\ldots,x[n]$ on a *grid*. The *finite difference* of two adjacent values on a *uniform* grid is defined by the relation
