...
function OUTSCH_WKB(E::T, grid::Grid{T}, def::Def{T}) where T<:Real
    ...
    n > 0 || error("Error: OUTSCH_WKB requires non-zero lower cl.t.p.")

    p = sqrt.(abs.(v .+ s .- E))                      # quasi-classical momentum
    I = [grid_trapezoidal_integral(p, i:n, grid) for i=1:n]
    ...
    Na > k+1 || error("Error: quasi-classical approximation fails (Na ≤ k+1)")
