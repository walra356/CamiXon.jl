...
```
"""
function mendeleev(Z::Int; dict=dictElements)

      elt = (name, symbol, weight) = get(dict, Z, nothing)
...
```
"""
function nucleardata(Z::Int, A::Int; dict=dictIsotopes)

    iso = (symbol, radius, mass, I, π, lifetime, mdm, eqm, ra) = get(dict, (Z, A), nothing)
...
#### Examples:
```
isotope = castIsotope(Z=1, A=1, msg=false)
...
atom = castAtom(Z=1, A=1, Q=0, msg=false)
  Atom(1, 1, 0, 1, Element("hydrogen", "H", 1.008), Isotope(1, 0, 1, 0.8783, 1.007825032, 1//2, 1, 1.0e100, 2.792847351, 0.0, 99.9855))
