...
function adams_moulton_solve!(Z::Vector{Complex{T}}, E::T, grid::Grid{T}, def::Def{T}, adams::Adams1{T}) where T<:Real
    ...
    adams = updateAdams1(adams, E, grid, def)
...
function adams_moulton_nodes(E::Real, scr::Vector{T}, grid::Grid{T}, def::Def{T}; imax=25, msg=true) where T<:Real
    ...
    adams = castAdams1(init.E, grid, def)
...
function adams_moulton_precise!(Z, init, grid, def, adams; imax=10, ϵ=1e-6, msg=false)
    ...
    grid = autoGrid1(def.atom, def.orbit, B; Ntot=grid.N , Rmax, k, msg)

    def = castDef(grid, def.atom, def.orbit, def.codata; def.pos, scr, msg=false)
    adams = castAdams1(init.E, grid, def)
